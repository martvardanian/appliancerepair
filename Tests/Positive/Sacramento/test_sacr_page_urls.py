...
from Locators.locators import SacramentoLocators, GenericLocators
...
class Test_pages(BaseTest):
    def test_check_header_footer_urls_and_titles(self, setup):
        try:
            self.driver = setup
            self.homePage = HomePage(self.driver, Urls.Sacramento)
            ...
            assert 'Service Areas' in self.basePage.get_text_from_index(SacramentoLocators.page_title, 1)

            self.homePage.click_header_link(GenericLocators.services_header)
            assert 'services' in self.driver.current_url
            assert 'Services' in self.basePage.get_text_from_index(SacramentoLocators.page_title, 1)

            self.homePage.click_header_link(GenericLocators.brands_header)
            assert 'brands' in self.driver.current_url
            assert 'Brands' in self.basePage.get_text_from_index(SacramentoLocators.page_title, 1)

            self.homePage.click_header_link(GenericLocators.contact_header)
            assert 'contact-us' in self.driver.current_url
            assert 'Contact Us' in self.basePage.get_text_from_index(SacramentoLocators.page_title, 1)

            self.homePage.click_header_link(SacramentoLocators.menu_logo)
            assert 'home' in self.driver.current_url
            assert 'Instant & Quality Appliance Repair Services' in self.basePage.get_text(SacramentoLocators.banner_title)
            ...
            assert 'Schedule Service' in self.basePage.get_text_from_index(SacramentoLocators.page_title, 1)
    ...
    def test_check_single_pages_urls_and_titles(self, setup):
        try:
            ...
            assert 'Dishwasher Repair' in self.basePage.get_text(SacramentoLocators.service_title)

            self.homePage.click_popup_service_active(GenericLocators.popup_wine)
            assert 'wine-cooler-repair' in self.driver.current_url
            assert 'Wine Cooler Repair' in self.basePage.get_text(SacramentoLocators.service_title)
            ...
            assert 'Trash Compactor Repair' in self.basePage.get_text(SacramentoLocators.service_title)
            ...
            assert 'Garbage Disposal Repair' in self.basePage.get_text(SacramentoLocators.service_title)
        finally:
            self.driver.close()
    ...
    def test_check_card_and_title(self, setup):
        try:
            ...
            cart_title = self.homePage.get_text(SacramentoLocators.cart_title)
            self.homePage.click_js(SacramentoLocators.cart_title)
            time.sleep(5)
            assert cart_title in self.basePage.get_text(SacramentoLocators.service_title)
